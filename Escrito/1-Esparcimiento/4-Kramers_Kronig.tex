...
Si bien el estudio del esparcimiento y la absorción de partículas suele realizarse a través de las secciones transversales, las propiedades ópticas dependen del material, el cual puede caracterizarse mediante su función dieléctrica $\varepsilon(\omega)$. Estas propiedades se determinan experimentalmente mediante distintas técnicas espectroscópicas, como la espectroscopía de transmisión, basada en la ley de Beer–Lambert, en la cual las características del material se analizan a partir de la altura, el ancho y la posición de los picos espectrales \cite{KramersKronigRelationsSum2005}. Sin embargo, esta técnica solo proporciona información de la parte imaginaria de la función dieléctrica \cite{KramersKronigRelationsSum2005}. Aunque existen técnicas avanzadas de caracterización óptica, como la espectroscopía elipsométrica, que permite obtener información sobre las partes real e imaginaria de la función dieléctrica mediante medidas de reflectancia y fase, su implementación experimental y el análisis de datos suele ser complejo \cite{fujiwaraSpectroscopicEllipsometryPrinciples2007}. Una alternativa de abordar esta limitación es mediante el principio de causalidad, que permite obtener información complementaria a partir de los datos experimentales \cite{KramersKronigRelationsSum2005}. A partir del pirncipio de causalidad y de la analiticidad de una función óptica lineal compleja como: la susceptibilidad, la función dieléctrica, el índice de refracción o la reflectividad, se deducen las relaciones de Kramers–Kronig (KK), las cuales describen la conexión entre las partes real e imaginaria de dicha función \cite{KramersKronigRelationsSum2005}. En esta sección se considera un material homogéneo e isótropo caracterizado por una función óptica lineal compleja, en este caso, la función dieléctrica, definida en un espacio de frecuencias complejo.
...
\end{equation}
%
respectivamente. Al sustituir la Ec. \eqref{eq:d3} en la transformada de Fourier inversa de las Ecs. \eqref{eq:TFD}, se obtiene
%
\begin{equation}
...
\end{align}
%
donde además se consideró que los órdenes de integración se pueden intercambiar dado que la integración se realiza en el mismo intervalo ($-\infty$, $\infty$).

\noindent Al reescribir el campo eléctrico como\footnote{Donde se realiza el cambio de variable $\tau = t - t'$ y se emplea la función constante \cite{arfkenMathematicalMethodsPhysicists2011a}
	%
\begin{equation}
...
\end{equation}
%
sumar $\vb{E}(\vb{r},t')-\vb{E}(\vb{r},t')$ y multiplicar por $\varepsilon_0/\varepsilon_0$ a la Ec. \eqref{eq:TF_D_intermedia_2}, simplificando se obtiene
%
\begin{equation}
