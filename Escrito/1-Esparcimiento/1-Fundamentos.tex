...
\end{equation}
%
donde $\vb{P}$ es la polarización y $\vb{M}$ es la magnetización, que corresponden al momento dipolar eléctrico y magnético por unidad de volumen, respectivamente. La relación lineal más general entre $\vb{E}$ y $\vb{D}$ está dada por \cite{jacksonClassicalElectrodynamics2021a}
%
...
\noindent de modo que la Ec. \eqref{eq:d1} es una convolución\footnote{La convolución de dos funciones $f(t), g(t)$ está dada por $f\ast g=\int_{-\infty}^{\infty}f(t-t')g(t')\dd{t'}$ \cite{arfkenMathematicalMethodsPhysicists2011a}.} en el tiempo. Al aplicar la transformada de Fourier y el teorema de la convolución\footnote{El teorema de la convolución es $\mathscr{F}[f\ast g]=\mathscr{F}[f]\mathscr{F}[g]$, donde $\mathscr{F}$ es la transformada de Fourier de una función. En este trabajo se considera la convención de la transformada de Fourier como \cite{arfkenMathematicalMethodsPhysicists2011a}
	...
	\end{equation*} 
donde la dependencia indica si se trata de la transformada de Fourier directa (dependencia en la frecuencia) o inversa (dependencia en el tiempo).}, la Ec. \eqref{eq:d1} se reescribe como un producto en el espacio de frecuencias y, asumiendo las mismas consideraciones para los campos $\vb{B}$ y $\vb{H}$, se llega a un resultado análogo. Esto es \cite{bohrenAbsorptionScatteringLight2008}
%
\begin{tcolorbox}[ams align]
...
\end{align}
%
donde $\chi$ y $\chi_m$ son cantidades adimensionales. Al sustituir las Ecs. \eqref{eq:pol_mag} en las Ecs. \eqref{eq:D_H} y al comparar con las Ecs. \eqref{eq:d3_b3} se obtiene que
%
\begin{equation}
