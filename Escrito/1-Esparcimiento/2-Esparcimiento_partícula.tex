...
El problema de esparcimiento por una partícula de tamaño, forma y propiedades ópticas específicas, iluminada por una onda monocromática polarizada arbitrariamente, consiste en determinar el campo electromagnético en todos los puntos de la partícula y en todos los puntos del medio homogéneo en el que está incrustada.
...
\noindent Considerando la descomposición del vector de Poynting total en tres términos \cite{Bohren}
\begin{equation}
	...
	\label{S}
\end{equation}
asociados al campo incidente, al campo esparcido y a la interacción entre los dos anteriores, respectivamente y donde Re$\{\cdot\}$ denota la parte real de un número complejo, se tiene que $W_{abs}=W_i-W_s+W_{ext}$, donde \cite{Bohren}
\begin{align}
	W_i  &= -\int_{A}\langle\Vec{S^i}\rangle_t\cdot\hat{e}_r\text{ dA},\label{Wi}\\ W_s&=\int_{A}\langle\Vec{S^s}\rangle_t\cdot\hat{e}_r\text{ dA}, \label{Ws} \\ 
	W_{ext} &= -\int_{A}\langle\Vec{S^{ext}}\rangle_t\cdot\hat{e}_r\text{ dA},\label{Wext} 
\end{align}
